...
**Convention (enforced):** every `SUBTYPE name` must be identical to its `ChemistryID`. This is required for the EVA upgrade system to function correctly — `EVAupgrade` uses the target subtype name as a direct key for chemistry lookup.

`EVAupgrade` must be set explicitly to `none` on subtypes that should not allow an upgrade, because B9PartSwitch does not reset fields that are absent from a `DATA` block — the previous subtype's value would persist.
