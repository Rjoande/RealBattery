...
@PART
{
    RBvolume = #$/RESOURCE[ElectricCharge]/maxAmount$
    @RBvolume *= 0.3
...
MODULE
{
    name = RealBattery
    ChemistryID = PbAc
    EVAupgrade  = none // target SUBTYPE name; only required when using B9PartSwitch
}
```

To define custom chemistries via `REALBATTERY_CHEMISTRY` nodes or inline parameters, see the [Wiki](https://github.com/Rjoande/RealBattery/wiki).
...
```
    MODULE
    {
        ...
        uiGroupName = RealBatteryInfo
        uiGroupDisplayName = #LOC_RB_PAWgroup
...
**Convention (enforced):** every `SUBTYPE name` must be identical to its `ChemistryID`. This is required for the EVA upgrade system to function correctly â€” `EVAupgrade` uses the target subtype name as a direct key for chemistry lookup.

`EVAupgrade` must be set explicitly to `none` on subtypes that should not allow an upgrade, because B9PartSwitch does not reset fields that are absent from a `DATA` block â€” the previous subtype's value would persist.
